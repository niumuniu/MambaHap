...
def mambaHap_loss(Mamba_output: torch.Tensor,
    ...
    obj_reg =  torch.sum(Wdynamic * Wmask * (Mamba_output - Wover)**2)

    pos_loss = (1/(2*Mamba_output.shape[0])) * y * torch.pow(1 - Mamba_output, 2)
    neg_loss = (1/(2*Mamba_output.shape[0])) * (1 - y) * torch.pow(torch.clamp(Mamba_output - beta1, min=0.0), 2)
...
def train_MambaHap(outhead: str,
    ...
    for epoch in range(num_epoch):
        ...
        if MambaHap_savefile and (epoch % 10 == 0):
            checkpoint = {'epoch': epoch + 1, 'embed_ae': embedAE.state_dict(), 'MambaHap': mamba.state_dict(), 'optimizer': MambaHap_optimizer.state_dict()}
