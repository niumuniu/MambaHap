...
from main_Mambahap import caculate_parameters,assignment,optimise,MambaHap_loss
from embeddings import AE_train
from utils import *
from mambapy.mamba import Mamba, MambaConfig

def train_MambaHap(SNVdata: SparseSNVMatrixDataset,
                  gpu: int=-1, 
                  hidden_dim: int = 128, 
                  num_hap: int = 2,
                  num_epoch: int = 2000, 
    ...
    batch_size = int(np.ceil(len(SNVdata)/20))
    ...
    total_params = (sum(p.numel() for p in embedAE.parameters()) + sum(p.numel() for p in mamba.parameters()))
    print('Total number of params: ', total_params)
    mamba_optimizer = optim.AdamW(list(mamba.parameters()) + list(embedAE.parameters()),lr=learning_rate)
    ...
    for epoch in range(num_epoch):
        ...
        for batch_data, batch_idx in dataloader:
            ...
            retnet_train_loss += mamba_loss.item()
...
def parser():
    ...
    parser.add_argument("-g", "--gpu", help='Number of GPUs to run MambaHap',default=0, type=int)
...
if __name__ == '__main__':
    ...
    savepath = 'data/NA12878/' + args.filehead + '/' +  args.filehead + '_MambaHap_hap_matrix.npz'
    ...
    def train_MambaHap_map(spos, SNVdata, gpu=args.gpu, num_runs=args.algo_runs):
        mec_min = np.inf             
        hap_matrix_best = np.zeros((args.ploidy, chunk_size), dtype=int)     
        for r in range(num_runs):    

            hap_matrix_run, mec_run = train_MambaHap(SNVdata,
                                                    gpu=gpu,
                                                    hidden_dim=128,
                                                    num_hap=args.ploidy, 
                                                    num_epoch=2000)
    ...
    for d in range(0, len(SNV_matrix_list), args.gpu):
        ...
        print('Running MambaHap on chunks starting at: ', chunk_starts)
        res_d = pool.starmap(train_MambaHap_map, zip(chunk_starts, chunk_SNVdata, gpu_list))
        print('Finished running MambaHap on chunks starting at: ', chunk_starts)
        pos_list, hap_chunk_list = zip(*res_d)

        output_dir = f'data/NA12878/{args.filehead}/hap_chunk'
        os.makedirs(output_dir, exist_ok=True)

        for pos, hap_chunk in zip(pos_list, hap_chunk_list):
            np.savez(os.path.join(output_dir, f'hap_pos_{pos}.npz'), hap_chunk)
            print('Chunk starts')
    ...
    MambaHap_res = np.load(savepath)
    MambaHap_hap = MambaHap_res['hap']
    mec_MambaHap = MEC(SNV_matrix, MambaHap_hap)
    nblocks = np.sum(np.sum(MambaHap_hap != 0, axis=0) == 0) + 1
    print("The MEC for MambaHap: ",mec_MambaHap)
    print("The number of blocks for MambaHap: ",nblocks)
