...
class ClusteringLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        return q
def calculate_mec(SNVmatrix_chunk, haplotypes_chunk):
    """
    SNVmatrix_chunk: shape (num_reads_in_chunk, chunk_size), 读段SNP矩阵，0表示缺失
    haplotypes_chunk: shape (ploidy, chunk_size), 单倍型矩阵
    返回：MEC分数（int）
    """
    num_reads = SNVmatrix_chunk.shape[0]
    ploidy = haplotypes_chunk.shape[0]
    mec = 0

    for i in range(num_reads):
        read = SNVmatrix_chunk[i]
        # 计算该read和所有haplotype的错配数，取最小值
        min_mismatch = float('inf')
        for h in range(ploidy):
            hap = haplotypes_chunk[h]
            # 找出read非缺失位点索引
            observed_positions = (read != 0)
            # 计算错配数（非缺失位点下不相等的个数）
            mismatch = np.sum((read[observed_positions] != hap[observed_positions]))
...
def train_caecseq(zonename, ploidy, expt, sparse=False):
    ...
    SNVmatrix_name = zone_name +'/'+zone_name + '_SNV_matrix.txt' # SNP matrix file name
    if not sparse:
        ...
        chunk_starts = list(range(0, SNVmatrix_full.shape[1], chunk_size - chunk_overlap))

        haplotypes = np.zeros((ploidy, SNVmatrix_full.shape[1]))  # Complete haplotypes
        print("Number of chunks: %d" % len(chunk_starts))

        recon_end = 0
        total_mec = 0  # 新增累计MEC分数变量
        t_0 = time.time()
        for i, cs in enumerate(chunk_starts):
            ...
            # 计算当前chunk的MEC分数
            mec_chunk = calculate_mec(SNVmatrix_chunk[chunk_rows], chunk_haplotypes)
            total_mec += mec_chunk
            print(f"MEC score for chunk {i}: {mec_chunk}")

        print("Finished running on %d chunks in %d seconds." % (len(chunk_starts), time.time() - t_0))
        print("Total MEC score:", total_mec)
