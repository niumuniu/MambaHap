...
def train_caecseq(zonename, ploidy, expt,check_swer:bool = False):


    #gt_file = zonename + '/combined.fa'
    pos_file = zonename + '/' + zonename + '_SNV_pos.txt'

    if check_swer:
        true_haplo = read_true_hap(gt_file, pos_file)
    zone_name = zonename # zone name
    SNVmatrix_name = zonename + '/' +zone_name + '_SNV_matrix.txt' # SNP matrix file name
    ...
    if check_swer:
        ...
        swer_best = SWER(haplotypes, true_haplo)
        print(f"CAECSeqSWER:{swer_best}")
