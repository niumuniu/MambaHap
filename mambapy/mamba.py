...
class Mamba(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.ResidualBlock1 = nn.ModuleList([
        ...
        self.attn_layers = nn.ModuleList([
            nn.MultiheadAttention(config.d_model, num_heads=4, batch_first=True)
            for _ in range(config.n_layers)
        ])
    ...
    def forward(self, x):  # x: [B, L, D]
        for i in range(self.config.n_layers):
            # Mamba卷积块
            y = self.ResidualBlock1[i](self.ln1[i](x)) + x

            # Attention 融合
            attn_out, _ = self.attn_layers[i](y, y, y)
            y = y + attn_out  # 再次残差连接

            # FFN 层
            x = self.mlp[i](self.ln2[i](y)) + y
